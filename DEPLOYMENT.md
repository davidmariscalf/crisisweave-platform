...
Keep backups off-host as well as on persistent storage, encrypt them, restrict access, and rehearse restoration.
...
The application itself also emits baseline security headers and request IDs. These are defense in depth, not a replacement for reverse-proxy policy.
